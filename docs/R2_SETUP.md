...
**In HTML layout**:
```html
<link rel="icon" type="image/png" href="/XAOSTECH_LOGO.png" />
```

**In Worker Route Handler** (example: xaostech.io):
```typescript
// In src/worker.ts
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/XAOSTECH_LOGO.png' || url.pathname.startsWith('/img/')) {
      const filename = url.pathname === '/XAOSTECH_LOGO.png' 
        ? 'XAOSTECH_LOGO.png' 
